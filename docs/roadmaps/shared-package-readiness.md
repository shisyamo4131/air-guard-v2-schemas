...
| Repeatable package validation and test baseline | 25 | 0 | In progress | Formal fail-closed eleven-file runner, maintained error-definition assertions, successful Node 22/24 workflow matrix, and release guard are implemented. Published 3.0.0-dev.3 has successful Node 22/24 formal-suite and Node 24 release-guard evidence; supported Node range and complete compatibility baseline remain open, so no predefined full gate is complete |
| Release, publish, rollback, and consumer integration runbook | 15 | 0 | In progress | Exact 3.0.0-dev.3 commit/tag, workflow jobs, registry bytes/content, tagged-commit comparison, and peer-inclusive fresh imports and behavior are verified. The whole milestone gate remains incomplete because corrected-version adoption/build acceptance and rollback exercise remain pending; no predefined partial sub-gate exists |
...
1. Obtain separate consumer approval before AirGuardV2 adopts exact published and content-verified 3.0.0-dev.3.
...
- Two-consumer adoption of the published corrected Company/CCB contract remains pending; no consumer adoption is authorized by publication alone
