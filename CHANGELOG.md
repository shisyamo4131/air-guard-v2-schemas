...
- Corrected `SiteOperationSchedule.notify()` to copy each worker's `isStartNextDay` value into the created `ArrangementNotification.actualIsStartNextDay` field.
- Added a Firestore-free regression test covering the complete notification-creation path for a next-day start.
